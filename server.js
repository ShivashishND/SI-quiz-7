...
app.get('/say', (req, res) => {
  axios
    .get(
      'https://3g38ezpu4l.execute-api.us-east-1.amazonaws.com/test/say-no-kw'
    )
    .then((result) => {
      res.status(200)
...
app.get('/avg_outstanding', async function (req, res) {
  const sql_query =
    'select WORKING_AREA, avg(OUTSTANDING_AMT) as Average_Outstanding from customer group by WORKING_AREA'
...
app.post('/order/:id', async function (req, res) {
  ...
  const rows = await pool.query(create_query, [
    ord_num,
    ord_amount,
    advance_amount,
    ord_date,
